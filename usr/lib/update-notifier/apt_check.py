...
def get_distro():
    " get distro name "
    try:
        return _get_info_from_os_release(key="VERSION_CODENAME")
    except Exception:
...
def _handleException(type, value, tb):
    sys.stderr.write("E: Unknown Error: '%s' (%s)" % (type, value))
    sys.exit(-1)
...
def isSecurityUpgrade(ver):
    ...
    security_pockets = [("gNewSense", "%s-security" % DISTRO),
...
def write_human_readable_summary(outstream, upgrades, security_updates):
    ...
    if any([upgrades, security_updates]):
        outstream.write("\n")
...
def run(options=None):
    ...
    try:
        ...
        sys.stderr.write("E: Error: Opening the cache (%s)" % e)
        sys.exit(-1)
    ...
    if depcache.broken_count > 0:
        sys.stderr.write("E: Error: BrokenCount > 0")
        sys.exit(-1)
    ...
    try:
        ...
        sys.stderr.write("E: Error: Marking the upgrade (%s)" % e)
        sys.exit(-1)
...
if __name__ == "__main__":
    ...
    parser.add_option("-p",
                      ...
                      help="Show the packages that are "
                             "going to be installed/upgraded")
    parser.add_option("",
                      "--human-readable",
                      action="store_true",
                      dest="readable_output",
                      help="Show human readable output on stdout")
    parser.add_option("",
                      "--security-updates-unattended",
                      action="store_true",
                      help="Return the time in days when security updates "
